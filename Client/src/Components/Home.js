  ...
  const handleTodoSubmit=(e)=>{
    ...
    auth.onAuthStateChanged(user=>{
      if(user){
        ...
        }).then(console.log).catch(err=>setTodoError(err.message))
        times.forEach((time)=>{
          db.collection('notifications').add({
            Todo: todo,
            description: description,
            whenToNotify: (time.$d > Date.now())?time.$d:time.add(1,'day').$d,
...
const deleteTime = (time)=>{
  setTimes(times.filter((time1)=>time1 !== time));
}
    return (
          ...
          <div className='container'>
            ...
            {currentUser&&<>
              <input type="text" placeholder="Enter the medicine name"
                className='form-control my-4' required
              ...
              <div onClick={addTime} className='btn btn-primary mx-3'>Add time</div>
              </div>
              <div className='d-flex'>
                {times.map((time)=>(<div className="timestamp" key={time}><p>{`${time.hour()}`.length < 2?'0'+time.hour():time.hour()}:{`${time.minute()}`.length < 2?'0'+time.minute():time.minute()}</p><Icon className='icon' onClick={()=>deleteTime(time)} size={18} icon={trash}/></div>))}
              </div>
              <br></br>
              <div style={{width: 100+'%',
            ...
            {!currentUser&&<>
              <input type="text" placeholder="Enter medications"
